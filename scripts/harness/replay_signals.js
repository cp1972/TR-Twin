...
(async()=>{
  const COH=fs.existsSync('examples/working-class-authors/data/actors/cohorte_contrat.csv')?'examples/working-class-authors/data/actors/cohorte_contrat.csv':'cohorte_contrat.csv';
  await sleep(150);
  if(errors.length){console.log("✗ erreurs chargement:",errors[0]);process.exit(1);}
  const cohort=fs.readFileSync(COH,'utf8');
  const insts=fs.readFileSync('examples/working-class-authors/data/instances/instances_probe_demo.csv','utf8');
  ...
  await loadFile('probefile',insts,'examples/working-class-authors/data/instances/instances_probe_demo.csv');
