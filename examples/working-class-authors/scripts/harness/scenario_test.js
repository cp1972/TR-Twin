...
setTimeout(()=>{
  ...
  feed('contractfile',fs.readFileSync('examples/working-class-authors/data/actors/cohorte_contrat.csv','utf8'));
  feed('instcohortfile',fs.readFileSync('examples/working-class-authors/data/instances/instances_full_empirical.csv','utf8'));
  ...
  // helper: lance un scenario, renvoie [readHTML, tlend]
  ...
  // 2) l'AMPLEUR change le resultat
  ...
  // 3) l'HORIZON change le resultat ET la frise
  ...
  // 4) controle joue aussi (tendance de controle)
  ...
  // 5) determinisme (memes params 2x)
  ...
  // 6) RESET restaure la frise historique
  ...
  // 7) carte + replay (relance, frise re-prolongee)
