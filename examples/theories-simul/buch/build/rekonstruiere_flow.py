...
in report/anhang/code_structure_seq.py, danach werden je Akteur die Wechsel
zwischen aufeinanderfolgenden Etappen gezählt.

Gegenprobe: die daraus abgeleiteten Größen S, g und act müssen mit
report/anhang/tr_measures.json übereinstimmen. Das Skript prüft das und bricht
ab, wenn es nicht stimmt.

    python3 rekonstruiere_flow.py <repo>/examples/working-class-authors
"""
...
def main():
    ...
    stages_pfad = os.path.join(anhang, 'stages_long_final.csv')
    ziel = os.path.join(anhang, 'flow.json')

    SS, head_concept = lexikon(os.path.join(anhang, 'code_structure_seq.py'))
    ...
    ref = json.load(open(os.path.join(anhang, 'tr_measures.json'), encoding='utf-8'))
