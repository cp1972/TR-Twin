...
(async()=>{
  await sleep(150);
  const cohort=fs.readFileSync('data-mode/data/actors/cohorte_contrat.csv','utf8');
  const full=fs.readFileSync('data-mode/data/instances/instances_full_empirical.csv','utf8');
  await load('contractfile',cohort,'cohorte.csv');
  await load('instcohortfile',full,'instances_full.csv');
  const rows=w.runSeries();
  const years=rows.map(r=>r.year);
  // Präsenz der Akteure je Struktur (die transversale Zirkulation, sichtbar gemacht)
  ...
  // Vermittlungsstärke je Struktur (größtes Gewicht der Nicht-Kontrollinstanzen)
  ...
  // Kurzbericht: Präsenzhöhepunkt in A und D, Korrelation Präsenz~(Vermittlung+Kontrolle)
  const NM=['A','B','C','D'];
